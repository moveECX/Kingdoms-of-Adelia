...
## Aktueller Kontext
**Stand (2026-05-25): Phase 1–5 abgeschlossen — komplette Roadmap. Das Spiel ist von Aufbau über PvP bis zum Allianz-Sieg (L10-Paläste aller acht Tugenden) im Browser spielbar.**
...
- **Phase 5 (Endgame):** 8 **Tugenden** (`shared/constants/game.ts`); Migration 0006 (shrines/palaces/world_state); **Schreine** erleuchten nahe Allianzstädte → **Paläste** (1/Stadt, max L10, `game/endgame.ts buildPalace`); allianzweite **Faith** pro Tugend; **Sieg = L10-Palast aller acht Tugenden** → `world_state.ended` + Champion; **Ranglisten** (Spieler/Allianzen). REST `/endgame`, `/cities/:id/palace`, `/leaderboard`. Client: `EndgamePanel.svelte` (Faith, Palast-Bau, Schreine, Ranglisten, Sieg-Banner). Seed mit 8 Schreinen.
- Tests: **38 Unit-Tests** (inkl. Nachtschutz/Türme/Fallen) + Smoke-Skripte (`server/src/dev/*-smoke.ts`): build/train/raid, `ws-smoke`, `resume-smoke`, `map-smoke`, `pvp-smoke`, `siege-smoke`, `market-smoke`, `alliance-smoke`, `endgame-smoke` (Sieg).
...
**Als Nächstes (Roadmap durchgespielt + Roster vollständig baubar — verbleibende Politur):** Balancing der Dev-Werte `[A]`; Eindeutschung der Anzeigenamen; Phase-4/5-Vereinfachungen (Support-Angriff, Schiff/Cap, Schrein-Zeit-Aktivierung, „castled"/3×3-Palast-Slot); Kysely-0.29-Upgrade; Prod-Build auf `dist/*.js`. **Demo-Seed:** dev = Hall L8 + Citadel/Market/Sanctuary/Mage Tower/Stable/Warehouse + 300 Berserker/150 Ranger/1 Marshal + Allianz ADL; aldara/borin ohne Schild + Garnison → PvP/Markt/Endgame sofort testbar. Login: `dev` / `password123`.
...
- **Phase-4/5-Vereinfachungen (Balancing/Content, bewusst):** Roster ist vollständig baubar (**34 Gebäude, 18 Einheiten**); Stats/Kosten/Zeiten der späten Gebäude/Einheiten sind Dev-Werte `[A]`. Support-Angriff fehlt; Markt-Transfer nutzt Karren-Tempo ohne Schiff/Cap-Unterscheidung; Reisezeit dev-pauschal 60 s/Tile; Eroberung übernimmt eine geleerte Garnison; Schreine beim Seed aktiv (keine Zeit-Aktivierung); Palast ohne „castled"/3×3-Slot-Check; Turm-Kapazitäten level-unabhängig; Mutationen ohne DB-Transaktion (bei PvP-Concurrency absichern).
