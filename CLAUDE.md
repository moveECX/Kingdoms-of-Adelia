...
## Aktueller Kontext
**Stand (2026-05-25): Phase 1–3 abgeschlossen; Phase 4 begonnen — PvP-Kampf (Scout/Plunder/Assault) server + client spielbar. Offen in Phase 4: Türme/Fallen, Belagerung/Eroberung, Marktplatz, Allianzen.**
...
- **Phase 4 (PvP & Wirtschaft, in Arbeit):** PvP-Angriffe **Scout/Plunder/Assault** (`server/src/game/pvp.ts`) — Citadel-Gate, Garnison-Kampf mit **City-Wall**-Modifikator + **Nachtschutz** (−40%, 22–10 Uhr UTC), Loot (Carry-gedeckelt), Verteidiger-Verluste, **Kampfberichte an beide Seiten** (`combat_reports` mit attacker_id+defender_id). Bewegungs-Helfer in `movement.ts`, Auflösungs-Routing in `military.ts` (raid + pvp). REST `/cities/:id/attack` + `/reports`. Client: Angriff von der Karte (Klick auf fremde Stadt → Ziel, Scout/Plündern/Assault), Berichte-Ansicht (`ReportsPanel.svelte`).
- Tests: **36 Unit-Tests** (inkl. Nachtschutz) + Smoke-Skripte (`server/src/dev/*-smoke.ts`): build/train/raid, `ws-smoke`, `resume-smoke`, `map-smoke`, `pvp-smoke` (Plünderung + Loot + Citadel-Gate).
...
**Als Nächstes (Phase 4 fortsetzen, `ROADMAP.md`):** **Türme/Fallen**-Modifikatoren (Formel-Hooks + Gebäude — Türme/Fallen sind noch nicht in `data/buildings.yaml`); **Belagerung & Eroberung** (Marshal, %/Welle, Stadtübernahme — braucht ggf. `siege_progress`-Migration); **Marktplatz** (Inserate + Karren/Schiff-Transfers nach Reisezeit); **Allianzen** (Ränge, Diplomatie, Allianz-Chat, Event-Log; `accounts.alliance_id` existiert bereits). Login: `dev` / `password123` (Seed).
